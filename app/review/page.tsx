...
export default async function ReviewPage({ searchParams }: ReviewPageProps) {
  ...
  return (
    <PageShell
      ...
      path="/review"
      eyebrow={locale === "ko" ? "주간 리뷰" : "Weekly review"}
      title={locale === "ko" ? "이번 주 흐름을 볼게요." : "Look for patterns, not proof."}
      description={
        locale === "ko"
          ? "잘된 점과 막힌 점만 간단히 봅니다."
    ...
    >
      ...
      <div className="grid gap-6">
        <Card className="bg-[var(--surface-strong)]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--primary)]">{locale === "ko" ? "어려웠던 점" : "What felt hard"}</p>
          <p className="mt-3 text-base leading-7 text-[var(--foreground)]">{weeklySummary.difficultMoments}</p>
        </Card>
        <Card className="bg-[var(--surface-strong)]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--primary)]">{locale === "ko" ? "도움이 된 점" : "What helped"}</p>
